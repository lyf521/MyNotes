...
function empunloadCity(option)
{
	option = jQuery.extend({
		sheng:"emp_Unload_province",		//省的网页ID
		shi:"emp_Unload_city",			//市的网页ID
		xian:"emp_Unload_country",		//县的网页ID
		xiang:"emp_Unload_street",		//乡的网页ID
		sheng_val:"",		//默认省份
		shi_val:"",			//默认地区
		xian_val:"",		//默认县
		xiang_val:""		//默认乡镇
	...
	var onchgCity1 = function()
	{
		...
		gpm.initCity3(document.getElementById(option.xiang), '', '', '');
		$("#"+option.xiang).hide();
		if($("#"+option.xian).children().length > 1){
			$("#"+option.xian).show();
		}else{
			$("#"+option.xian).hide();
		}
